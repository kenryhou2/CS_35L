...
int main(int argc, char **argv)
{
	...
	while (c != EOF) //traverse through the standard input.
	{
		int match = 0;
		int str_index;
		for (str_index = 0; str_index < strlen(from_operand); str_index++) //traverse through from_operand
		{
			...
			{
				match = 1;
				putchar(*(to_operand + str_index)); //if there is a match with the correct char, replace it with the corresponding char from the to_operand.
			}
		}
